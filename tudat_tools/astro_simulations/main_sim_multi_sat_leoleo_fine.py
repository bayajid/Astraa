...
# Simulation time
sim_time = 3600*2
# Time-step of orbital states
output_timestep_req = 1
# time-step for numerical integrator
fixed_step_size = 60

# sim_time = 3600
sim_start = 0
simulation_name = f'many_satellites'
...
# Start time : 2025, January, 12:00:00 

simulation_start_epoch = 2 * 365.25 * 24 * 3600 + sim_start #  x 365.25 days/yr * 24 hrs/day * 3600 sec/hr [seconds since J2000]
...
full_dep_var = 0 # add acceleration norms to dependent variables
...
ic_sats = np.array(
    [[7378e3, 0.0001, np.deg2rad(89), np.deg2rad(0.1), np.deg2rad(0), np.deg2rad(-90)],
    [7378e3, 0.0001, np.deg2rad(89), np.deg2rad(0.1), np.deg2rad(0), np.deg2rad(-80)],
    [7378e3, 0.0001, np.deg2rad(89), np.deg2rad(0.1), np.deg2rad(0), np.deg2rad(-70)],
    [7378e3, 0.0001, np.deg2rad(89), np.deg2rad(0.1), np.deg2rad(0), np.deg2rad(-60)],
    [7378e3, 0.0001, np.deg2rad(89), np.deg2rad(0.1), np.deg2rad(0), np.deg2rad(-50)],
    [7378e3, 0.0001, np.deg2rad(89), np.deg2rad(0.1), np.deg2rad(-5), np.deg2rad(-45)],
    [7378e3, 0.0001, np.deg2rad(89), np.deg2rad(0.1), np.deg2rad(175), np.deg2rad(90)],
    [7378e3, 0.0001, np.deg2rad(53), np.deg2rad(0.1), np.deg2rad(15), np.deg2rad(3)],
    [7378e3, 0.0001, np.deg2rad(53), np.deg2rad(0.1), np.deg2rad(15), np.deg2rad(7)]]
)
ic_const_all = np.zeros((n_sats_total,6))
...
independent_variable = np.arange(states_array[0,0], states_array[-1,0]+output_timestep_req, output_timestep_req)
states_interpolated = np.zeros((independent_variable.shape[0], states_array.shape[1]))
# Create settings for Hermite spline interpolation
linear_interpolation_settings = interpolators.cubic_spline_interpolation()
# Create interpolator
interpolator = interpolators.create_one_dimensional_vector_interpolator( states, linear_interpolation_settings)
# Interpolate data set in data_to_interpolate at t=100
for ii, t in enumerate(independent_variable):
    interpolated_value = interpolator.interpolate( t )
    states_interpolated[ii,0] = t
    states_interpolated[ii,1:] = interpolated_value
states_interpolated_dict = {row[0]: row[1:] for row in states_interpolated}
...
if save_outputs:
    ...
    save2txt(states_interpolated_dict, 'states_fine.dat', output_path)
    dict2txt(sim_params, 'simulation_parameters', output_path)   
    if len(dep_var_to_save) != 0: 
...
if plot_orbit_3d:
    ...
    f, ax = plt.subplots()
    for ii in range(n_sats_total):
        ax.plot(states_array[:,0], states_array[:, 6*ii+1], 'o-', label = f'{sat_names[ii]}_x')
        ax.plot(states_array[:,0], states_array[:, 6*ii+2], 'o-', label = f'{sat_names[ii]}_y')
        ax.plot(states_array[:,0], states_array[:, 6*ii+3], 'o-', label = f'{sat_names[ii]}_z')
    ax.legend()
    plt.show()
