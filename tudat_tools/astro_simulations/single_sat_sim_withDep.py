...
### SCRIPT TO SIMULATE A SINGLE SATELLITE
# with a basic initial condition interface
# and dependent variables of long lat etc

# Path variables
save_parent_dir = "simulation_output"
simulation_name = f'j2_leo_gt'
sat_name = 'leo_sat'
"""
...
"""
fixed_step_size = 10
hp_setting = 0 # High Precision simulation setting
j2_setting = 1  
# Load spice kernels
...
# Start time : 2024, January, 12:00:00 
simulation_start_epoch = 24 * 365.25 * 24 * 3600 + sim_start #  x 365.25 days/yr * 24 hrs/day * 3600 sec/hr [seconds since J2000]
...
elif j2_setting: # only include j2
...
ic_sats = np.array(
    [[7338e3, 0.0001, np.deg2rad(89), np.deg2rad(0.1), 0, np.deg2rad(0)]]
)
ic_const_all = np.zeros((len(ic_sats),6))
...
"""
dep_var_to_save = [propagation_setup.dependent_variable.longitude( sat_name, "Earth" ),
                   propagation_setup.dependent_variable.latitude( sat_name, "Earth" ),
                   propagation_setup.dependent_variable.heading_angle(sat_name, "Earth")
...
try:
    ...
    dependent_variables_available = 1
except:
    dependent_variables_available = 0
...
print(f'''
    ...
    ''')
if save_outputs:
    print(f'Saving results to {save_parent_dir}/{sat_name}')
    output_path = fr'orbital_simulations/{sat_name}/{simulation_name}'
    save2txt(states, 'state_history.dat', output_path)
    dict2txt(sim_params, 'simulation_parameters', output_path) 
    if dependent_variables_available:   
...
if plot_orbit_3d:
    ...
    ax.set_zlabel('z [m]')
    ax.legend()
    bplt.savefig(fig, name = f'3d_{simulation_name}', save_folder = f'{output_path}')
    plt.show()    