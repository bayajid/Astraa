...
save_append = "single_sat"
simulation_name = f'leo_hp_prop_1s_1d'
...
if save_outputs:
    ...
    if output_as_csv:
        try:
            # out_df = pd.DataFrame(data = states, columns = ['t_j2000', 'x', 'y', 'z', 'vx', 'vy', 'vz'])
            # out_df.to_csv(fr'orbital_simulations/{save_append}/{simulation_name}/states.csv', index = 0)
            # print(f'Saved as csv')
            df = pd.DataFrame(index=states.keys(),
                data=np.vstack(list(states.values())),
                columns = [
                     'x', 'y', 'z', 'vx', 'vy', 'vz'])
            df.to_csv(fr'orbital_simulations/{save_append}/{simulation_name}/orbital_states.csv', index_label= 't_j2000')
            print(f'Saved as csv')
        except:
            print(f'Failed to save as csv')
if plot_orbit_3d:
    # prepare to plot
