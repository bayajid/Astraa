### Updated code to simulate single-satellite constellation
...
# save_append = "terran_near_polar_split"
...
#TODO KP August 11 2023: Clean up constellation choise/design

## Set Simulation Parameters
# RAAN angular separation = 2pi/n_plaes (uniform) or 2pi/(2nplanes) (split)
constellation_type = 'split'
# constellation_type = 'uniform'

# whether to simulate only limited satellites
# shells_simulated = 'all' 
# shells_simulated = 'leo_i'
shells_simulated = 'leo_terran'
basic_acceleration = 1 # turn off MOST perturbations
...
simulation_name = f'NearPolar12x24{sim_time:.2f}h'
...
elif shells_simulated == 'leo_terran':
    # LIMIT nr of satellites simulates
    ta_0 = 0
    phasing_terran = 6 # 6 leads to 7.5 deg TA separation for 288 satellite const
    wdp_leo_terran = [89, 12, 24, phasing_terran] # incl [deg], Nr_planes [-], Nr_sats/plane [-], rel_phasing, total_sats
    wdp_leo_terran.append(int(wdp_leo_terran[1]*wdp_leo_terran[2]))
    shell_parameters_leo_terran = util.summarize_shell_params(wdp = wdp_leo_terran,
                                                   h=1050,
                                                   id='leo_t')
    const_params[1] = shell_parameters_leo_terran
sim_params = {}
...
ic_const_all, central_bodies, bodies_to_propagate, bodies, acceleration_settings = util.setup_constellations(
            ...
            eccentricity,
            constellation_shape=constellation_type,
            ta_0 = ta_0
        )
...
sim_params['state_ind'] = [ii*6 for ii, sat_name in enumerate(bodies_to_propagate)]
print(f'''
