...
moon_ilum_fractions = [0.3, 0.5, 0.7, 0.9, 1]
for moon_ilum_fraction in moon_ilum_fractions:
    ...
    for ii_loop, att_angle in enumerate(angles_desired):
        seed_used = ii_loop
        roll_required = att_angle/dt_data/nr_inputs*2
    ...
    df_title = f'ooc_{fct_used}_{title_append}_sample{nr_inputs}_datapt{n_samples}_spacing{spacing}'
    output_df = io.save_dict_2_csv(df_dict, df_title, subfolder = storage_folder)
    # nr_samples, datapt_used, datapt_spacing, weights?
    ...
    if 1:
        ...
        bplt.savefig(f, f'{case}_ooc_phasea_mor_triad_{moon_ilum_fraction}', subfolder =storage_folder)
