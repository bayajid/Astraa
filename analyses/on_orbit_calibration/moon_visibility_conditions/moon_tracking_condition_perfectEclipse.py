...
output_folder = r'outputs\tables\moon_conops_conditions\raw'
...
for ii, t_gps_ii in enumerate(t_gps):
    ...
    um, penum = where_sun.check_shadow_canonical(r_host[ii,:], r_sun_true)
    r_moon[ii,:] = r_moon_ii
...
# store
#%%
data = np.hstack((t_gps.reshape((nrows, 1)), r_host, r_moon, moon_illumination, np.array(bool_vis).reshape((nrows, 1)), moon_umbra, v_host))
df_stored = pd.DataFrame(data = data, columns = ['t_gps', 'x_h', 'y_h', 'z_h', 'x_m', 'y_m', 'z_m', 'illumination','is_visible', 'in_umbra', 'vx_h', 'vy_h', 'vz_h'])
