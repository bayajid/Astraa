...
if constrain_azel:
    ...
    for ii, att_prof in enumerate(attitude_profiles):
        quat_eci2bf, rot_eci2bf = att_sim.calc_quat_eci2bf(r_host, v_host, att_prof, t_gps = t_gps.flatten())
...
# TODO interpolate to finer data
...
err_ae_centroid = np.zeros((nrows, 2))
if add_noise:
...
# err_ae_centroid[]
# TODO - for attitude profiles, create scan simulations
# find long enough time-gaps with possible scans -> get corner datapoint as scan outputs
# input those into mounting offset resolution
for ii, ii_scan in enumerate(ii_scans):
    ...
    q_eci2bf_c = conv.convert_ea2quat(ea_eci2bf_command_all[ii,:3] + err_att_host[ii,:])
    q_mounting_offset_c = np.array([1,0,0,0]) # Unity quaternion initially used
    r_host_ii_t = r_host_ii
    r_moon_ii_t = r_moon_ii + err_r_moon[ii,:]
    q_eci2bf_t = conv.convert_ea2quat(ea_eci2bf_command_all[ii,:3])
    q_mounting_offset_t = quat_mounting_offset

    ae_moon_commanded = ae_calc.calc_ae_full(r_host_ii_c, r_moon_ii_t, q_eci2bf_c, q_mounting_offset_c)[0]
    ae_moon_true = ae_calc.calc_ae_full(r_host_ii_t, r_moon_ii_t, q_eci2bf_t, q_mounting_offset_t)[0]
    
    ae_moon_commanded_all[ii,:] = ae_moon_commanded[:2]
    ae_moon_true_all[ii,:] = ae_moon_true[:2]
#%% MO resolution
...
print(f'PE attitude : \n{vec_calc.get_pe_for_rot(quat_resolved, quat_mounting_offset)} urad')
print(f'''
