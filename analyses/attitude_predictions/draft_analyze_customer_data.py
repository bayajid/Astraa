...
# path jazz
path_cwd = os.getcwd()
path_cust_data = fr'{path_cwd}/analyses/attitude_predictions/cust_data' #/attitude_tools/attitude_predictions/cust_data'
path_outputs = fr'{path_cwd}/analyses/attitude_predictions/outputs' #/attitude_tools/attitude_predictions/outputs'
...
data_quat =  scipy.io.loadmat(full_quat_path)
data_array = data_quat[fname_quat[:-4]][0][0][0]
q_loaded = data_array[:,1:]
...
if plot_quat:
    ...
    axlabels = ['q1', 'q2', 'q3', 'q_scalar']
    for ii, ax in enumerate(axs):
        ax.plot(t_vec_loaded, q_loaded[:,ii])
        ax.set_ylabel(axlabels[ii])
    ...
    pass
if plot_ea_fromquat:
    q_scalarfirst = np.hstack((q_loaded[:,-1], q_loaded[:,:3]))
    quat_from_ea = conv.convert_ea2quat(ea_all)
    pass