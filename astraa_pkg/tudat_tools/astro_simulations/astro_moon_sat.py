# Code to simulate LEO and MEO satellite orbits for 
# moon trackign accessibility
# Date - 2023-06-16. Author - Kipras
#%%
...
# Start time : 2023, January, 12:00:00 

# sim_time = 1 # days
# sim_time = 7 # days
sim_time = 62 # days
# sim_time = 365 # days
simulation_start_epoch = 23 * 365.25 * 24 * 3600 # 22 years x 365.25 days/yr * 24 hrs/day * 3600 sec/hr
simulation_end_epoch = simulation_start_epoch + sim_time * 86400
# Set simulation labels
simulation_name = f'leomeo_mixincl{sim_time:.0f}d'
...
debug_mode = 0
sats_simulated = 'single_per_incl'
...
wdp_leo_polar = [89, 1, 1, phasing_param]  # incl [deg], Nr_planes [-], Nr_sats/plane [-], rel_phasing, total_sats
wdp_leo_polar.append(int(wdp_leo_polar[1]*wdp_leo_polar[2]))
h_leo_polar = 1000 # km
id_leo_polar = 'leo_polar'
wdp_leo_incl = [53, 1, 1, phasing_param] # incl [deg], Nr_planes [-], Nr_sats/plane [-], rel_phasing, total_sats
wdp_leo_incl.append(int(wdp_leo_incl[1]*wdp_leo_incl[2]))
h_leo_incl = 1000 # km
id_leo_incl = 'leo_incl'

wdp_leo_eq = [0, 1, 1, phasing_param] # incl [deg], Nr_planes [-], Nr_sats/plane [-], rel_phasing, total_sats
wdp_leo_eq.append(int(wdp_leo_incl[1]*wdp_leo_incl[2]))
h_leo_eq = 1000 # km
id_leo_eq = 'leo_eq'

wdp_meo = [0, 1, 1, phasing_param] # incl [deg], Nr_planes [-], Nr_sats/plane [-], rel_phasing, total_sats
...
shell_parameters_leo_incl = util.summarize_shell_params(wdp_leo_incl,
                                                   h_leo_incl,
                                                   id_leo_incl)
shell_parameters_leo_eq = util.summarize_shell_params(wdp_leo_eq,
                                                   h_leo_eq,
                                                   id_leo_eq)
...
const_params = {}
if sats_simulated == 'single_per_incl':
    const_params[1] = shell_parameters_leo_polar
    const_params[2] = shell_parameters_leo_incl
    const_params[3] = shell_parameters_leo_eq
    const_params[4] = shell_parameters_meo
...
if output_kepler_states:
    ...
    dep_var_to_save = []
dep_var_to_save.append(propagation_setup.dependent_variable.relative_position( "Moon", "Earth" ),
...
if plot_orbit_3d:
    ...
    ax.set_title(f'Simulated sats around Earth')

    for ii in range(n_sats_total):
        ax.scatter(states_array[:1000, 6*ii+1],
                states_array[:1000, 6*ii+2],
                states_array[:1000, 6*ii+3])
    ax.set_xlabel('x [m]')
    ax.set_ylabel('y [m]')
    ax.set_zlabel('z [m]')
    plt.show()    
