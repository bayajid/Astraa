...
t_gps_full_5ms = np.round(np.arange(t_gps[ii_truth_0], t_gps_end, dt_truth_req),3)
...
casename = f'test_pos_extap'

# get sub-second datapoints
t_rec_data = np.round(t_gps_full_5ms[ii_data_incoming_now],3)
# timestamp of the incoming data
t_stamp_data = np.round(t_gps_full_5ms[ii_data_timestamps],3)
...
if 0:
    ...
    s0 = s_data[0,:]
    t0 = t_stamp_data[0]
    # first long propagationB
    t1 = t0 + 11
    s1 = j2prop.integrate_rk4(j2prop.Bcalc_f_J2_separate, x_0=s0, t_vec_prop=[t0], t_step = 11, return_both=0)
