...
if save_outputs:
    ...
    if not use_new_saving_function:
        output_dict = {}
        output_dict['case_nr'] = test_nr
        output_dict['t_stamp_gps_s'] = in_t_gps_s
        output_dict['t_now_gps_s'] = in_t_now_s
        ...
        ## Make edits to include tnow and case nr
        pt_vec = pd.read_csv(f'unit_tests/pointing_testvectors.csv')
        pt_vec.insert(0, value= in_t_now_s, column = 't_gps_now')
        pt_vec.insert(0, value= test_nr, column = 'test_case')
        pt_vec.to_csv('unit_tests/pointing_testvectors.csv', index = 0)