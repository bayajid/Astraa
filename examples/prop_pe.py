...
propagate_selection = 'HOST'  #'TARGET'
...
prop_timeout = start_point + timeout_time
tolerance_km = 10
...
print(f"Starting Trajectory propagation at {pos_init}")
while prop_end <= prop_timeout:
    ...
    if i <= len(new_data) and new_data[i-1] <= start_point + prop_duration:
        ...
        update = True
        print(f'\nStarting from {t_init}\nNew data will arrive at: {prop_end}, after {prop_end -t_init} seconds, so propagating upto: {prop_end} [sec]')
    else:
    ...
    tspan = t_gps[t_init:prop_end:step]
    ...
    print(f'ITARATION: {i} Propagating from {tspan[0]} to {tspan[-1]}, for {tspan[-1]-tspan[0]} seconds')
...
# ax[0].plot((LOS_true[:, 0] - LOS_prop[:, 0])*1e3)
# ax[1].plot((LOS_true[:, 1] - LOS_prop[:, 1])*1e3)
# ax[2].plot((LOS_true[:, 2] - LOS_prop[:, 2])*1e3)
...
ax[0].set_title('Propagation position eroor [m]')
...
plt.figure()
plt.plot(1e6*np.arctan2(np.linalg.norm((LOS_true - LOS_prop)*1e3, axis=1), Link_distance_km*1e3))
